...
const handleStreamingError = (
  ...
  if (error.message && error.message.includes("Audio Timeout Error")) {
    console.log(
      `Audio timeout detected for session ${sessionId}, handling gracefully`
    );
      ...
      if (data.mode === "conversation") {
        ...
        switch (action) {
          case "start":
            ...
            recognizeStream.on("end", () => {
              console.log(
                `Transcription stream ended for speaker ${speakerId}`
              );
            });
        ...
        switch (data.type) {
          case "start":
            ...
            console.log(
              `Starting transcription session ${sessionId} from ${sourceLanguage} to ${targetLanguage}`
            );
            ...
            recognizeStream.on("end", () => {
              console.log(
                `Transcription stream ended for session ${sessionId}`
              );
            });
